...
function FilterBar({ filters=[], onAdd }) {
  return (
    <div style={{background:colors.surfaceFilter,borderRadius:6,padding:"10px 14px",display:"flex",alignItems:"center",gap:12,font:"400 13px/18px Roboto",color:colors.textPri,flexWrap:"wrap"}}>
      <span style={{fontSize:14,color:colors.textSec}}>🛒</span>
      {filters.map((f,i)=>(<span key={i} style={{background:"#fff",border:`1px solid ${colors.border3}`,borderRadius:4,padding:"4px 10px"}}>{f} ▾</span>))}
      <span onClick={onAdd} style={{color:colors.processing,font:"500 13px Roboto",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6}}>
