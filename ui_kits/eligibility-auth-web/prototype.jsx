...
function TopNav({ section, onSection, onLogo }) {
  ...
  return (
    <div style={{height:72,background:"linear-gradient(rgb(56,71,90) 0%, rgb(44,60,80) 42%, rgb(28,45,66) 100%)",display:"flex",alignItems:"center",padding:"0 32px",color:"#fff",gap:28,borderBottom:"1px solid rgba(255,255,255,.1)"}}>
      <img src="../../assets/neos-logo.png" style={{height:26,cursor:"pointer"}} alt="neos" onClick={onLogo} />
        ...
        {authRows.map((r,i)=>(
          <div key={i} onClick={()=>onRow&&onRow(r)} style={{display:"flex",height:54,padding:"0 8px 0 0",borderBottom:i===authRows.length-1?"none":"1px solid #EEF0F3",cursor:"pointer",position:"relative",alignItems:"center",background:review&&r.pd?"rgba(255,244,244,.6)":"#fff"}} onMouseEnter={e=>e.currentTarget.style.background="#F8FAFD"} onMouseLeave={e=>e.currentTarget.style.background=(review&&r.pd)?"rgba(255,244,244,.6)":"#fff"}>
            ...
            {cols.map((c,j)=>{
              let v=r[c.key];
              let content=v;
              if(c.key==="pd") content = v ? <DashedTag text={v} color="#C47097" /> : null;
              else if(c.key==="expiring" && v) content=<DashedTag text={v} color="#D7894A" />;
              else if(c.key==="expired" && v) content=<DashedTag text={v} color="#CF1322" />;
              else if(c.key==="attach") content=<span style={{color:"#8D949D",fontSize:15}}>🔗</span>;
              else if(c.key==="check") content=(<span style={{width:16,height:16,border:"1px solid "+(r.checked?"#1677FF":"#D8D8D8"),background:r.checked?"#1677FF":"#fff",borderRadius:2,display:"inline-flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11}}>{r.checked?"✓":""}</span>);
              else if(c.key==="name") content=<span style={{font:"500 13px/16px Roboto",color:"#3A424A"}}>{v}</span>;
              else if(!v) content=<span style={{color:"#8D949D"}}>-</span>;
              return (
